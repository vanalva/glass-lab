...
(function () {
  ...
  function applyPatternTransformToEl(el, s) {
    /* Escala → flat-mesh scale. Eased curve (quadratic top) so the default
       stays moderate but the high end reaches much larger meshes — fixes fine
       flat patterns like Acanalado being stuck small. 0.4×–4.0×. */
    var ps = (s.patternScale == null ? 50 : s.patternScale) / 100;
    var scale = 0.4 + ps * 1.6 + ps * ps * 2.0;
